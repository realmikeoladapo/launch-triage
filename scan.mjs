...
Options:
  ...
  --fail-on <severity>  Exit 2 on critical, high, or medium findings at/above the threshold
...
function gitFacts(root) {
  ...
  const gitRoot = resolve(topLevel.trim());
  const prefix = relative(gitRoot, root).replace(/\\/g, '/');
  ...
  return {
    ...
    exposure(targetPath, matcher, ruleId) {
      ...
      const head = run(['show', `HEAD:${repoPath}`]);
      ...
      const history = run(['log', '--all', '--format=', '-p', '--', repoPath]);
...
function buildReport({ client, product, preparedBy, reviewDate, root, findings, fileCount, audit, coverage }) {
  ...
  const coverageDetails = coverage.status === 'complete'
    ...
    : `This scan is incomplete and exits with code 1. Review or split the skipped paths, then rerun before relying on the result.\n\n${skippedRows.join('\n')}`;
...
function parseArgs(argv) {
  ...
  if (args.failOn) {
    ...
    if (!['critical', 'high', 'medium'].includes(args.failOn)) {
      throw new UsageError('--fail-on must be critical, high, or medium');
    }
  }
...
function main(argv = process.argv.slice(2)) {
  ...
  if (audit.status === 'failed' || coverage.status === 'partial') return 1;
  return thresholdMet(findings, args.failOn) ? 2 : 0;
}
